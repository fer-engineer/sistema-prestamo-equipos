    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white">
                @forelse($tipos_usuario as $tipo)
                <tr class="bg-white hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $tipo->id }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $tipo->nombre }}</td>
                    <td class="px-6 py-4 text-sm text-gray-600 max-w-xl truncate">{{ $tipo->descripcion }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <a href="{{ route('tipos_usuario.show', $tipo) }}" class="inline-flex items-center px-3 py-1.5 mr-2 bg-blue-100 text-blue-800 border border-blue-200 rounded hover:bg-blue-200 text-sm">Ver</a>
                        <a href="{{ route('tipos_usuario.edit', $tipo) }}" class="inline-flex items-center px-3 py-1.5 mr-2 bg-green-100 text-green-800 border border-green-200 rounded hover:bg-green-200 text-sm">Modificar</a>
                        <form action="{{ route('tipos_usuario.destroy', $tipo) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
