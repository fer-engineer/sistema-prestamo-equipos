...
<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8 border-b border-gray-200">
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-sm text-gray-500">Actualice la información del tipo de usuario.</p>
        </div>
...
<form action="{{ route('tipos_usuario.update', $tipo_usuario->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <!-- Campo Nombre -->
        <div class="space-y-2">
            ...
            <div class="relative">
                ...
                <input type="text" id="nombre" name="nombre" class="block w-full pl-12 pr-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('nombre') border-red-500 @enderror" placeholder="Ej: Administrador, Invitado" value="{{ old('nombre', $tipo_usuario->nombre) }}">
            </div>
            @error('nombre')
