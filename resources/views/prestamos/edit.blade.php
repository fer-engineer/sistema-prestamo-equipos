...
<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8 border-b border-gray-200">
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-sm text-gray-500">Actualice el formulario para modificar el préstamo.</p>
        </div>
    ...
    <form action="{{ route('prestamos.update', $prestamo) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <select id="persona_id" name="persona_id" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('persona_id') border-red-500 @enderror">
                    ...
                    @foreach($personas as $persona)
                        <option value="{{ $persona->id }}" {{ old('persona_id', $prestamo->persona_id) == $persona->id ? 'selected' : '' }}>
            ...
            <div class="space-y-2">
                ...
                <select id="encargado_id" name="encargado_id" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('encargado_id') border-red-500 @enderror">
                    ...
                    @foreach($encargados as $encargado)
                        <option value="{{ $encargado->id }}" {{ old('encargado_id', $prestamo->encargado_id) == $encargado->id ? 'selected' : '' }}>
            ...
            <div class="space-y-2">
                ...
                <select id="estado_id" name="estado_id" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('estado_id') border-red-500 @enderror">
                    ...
                    @foreach($estados as $estado)
                        <option value="{{ $estado->id }}" {{ old('estado_id', $prestamo->estado_id) == $estado->id ? 'selected' : '' }}>
                            {{ $estado->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="space-y-2">
                ...
                <input type="date" id="fecha_prestamo" name="fecha_prestamo" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('fecha_prestamo') border-red-500 @enderror" value="{{ old('fecha_prestamo', $prestamo->fecha_prestamo->format('Y-m-d')) }}">
            ...
            <div class="space-y-2">
                ...
                <input type="date" id="fecha_devolucion" name="fecha_devolucion" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('fecha_devolucion') border-red-500 @enderror" value="{{ old('fecha_devolucion', $prestamo->fecha_devolucion ? $prestamo->fecha_devolucion->format('Y-m-d') : '') }}">
            ...
            <div class="md:col-span-2 space-y-2">
                ...
                <textarea id="observaciones" name="observaciones" rows="4" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('observaciones') border-red-500 @enderror" placeholder="Agregar notas si es necesario...">{{ old('observaciones', $prestamo->observaciones) }}</textarea>
                @error('observaciones')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
