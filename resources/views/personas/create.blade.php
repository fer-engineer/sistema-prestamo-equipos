...
<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8 border-b border-gray-200">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Registrar Nueva Persona</h1>
            <p class="text-sm text-gray-500">Complete el formulario para agregar una nueva persona.</p>
        </div>
    ...
    <form action="{{ route('personas.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="telefono" name="telefono" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('telefono') border-red-500 @enderror" placeholder="Ej: 1234-5678" value="{{ old('telefono') }}">
                @error('telefono')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <select id="tipo_usuario_id" name="tipo_usuario_id" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('tipo_usuario_id') border-red-500 @enderror">
                    ...
                    @foreach($tiposUsuario as $tipoUsuario)
                        <option value="{{ $tipoUsuario->id }}" {{ old('tipo_usuario_id') == $tipoUsuario->id ? 'selected' : '' }}>{{ $tipoUsuario->nombre }}</option>
                    @endforeach
                </select>
